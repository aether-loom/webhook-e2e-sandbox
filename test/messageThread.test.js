...
describe('message thread state', () => {
  ...
  it('marks a non-empty composer as sending', () => {
    const state = updateDraft(createThreadState({ general: [] }), 'general', 'hello');
    const nextState = beginSending(state, 'general');

    assert.equal(getComposerState(nextState, 'general'), 'sending');
    assert.equal(beginSending(createThreadState({ general: [] }), 'general').sendingChannelIds.length, 0);
  });
