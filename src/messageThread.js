...
export function beginSending(state, channelId) {
  if (!getDraftForChannel(state, channelId).trim() || state.sendingChannelIds.includes(channelId)) {
    return state;
  }

  return {
    ...state,
    sendingChannelIds: [...state.sendingChannelIds, channelId],
  };
}
