...
function render() {
    ...
    <main class="slack-shell">
      ...
      <section class="conversation" aria-labelledby="channel-title">
        ...
        <form class="composer composer--${composerState}" data-composer-form>
          ...
          <textarea id="message-draft" rows="3" placeholder="Message #${activeChannel.name}" data-composer-input>${escapeHtml(draft)}</textarea>
          <div class="composer__footer">
            <span class="composer__state">${composerState === 'ready' ? 'Ready to send' : composerState === 'sending' ? 'Sending…' : 'Draft a message'}</span>
            <button class="send-button" type="submit" ${composerState === 'empty' ? 'disabled' : ''}>Send</button>
          </div>
        </form>
      </section>
    </main>
  ...
  app.querySelector('[data-composer-form]').addEventListener('submit', (event) => {
    ...
    threadState = addMessage(threadState, activeChannel.id, getDraftForChannel(threadState, activeChannel.id));
    render();
  });
